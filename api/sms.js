...
module.exports = async function handler(req, res) {
  ...
  // Caller may pass `from` to send from a different number on the same account.
  // Falls back to the env default if none provided.
  const from    = (fromOverride && /^\+\d{8,15}$/.test(fromOverride.trim()))
    ? fromOverride.trim()
    : process.env.SW_PHONE_NUMBER;
  ...
  try {
    ...
    res.json({ success: true, sid: data.sid, from });
  } catch (e) {
